...
{
	sf2d_vertex_pos_col *vertices = sf2d_pool_calloc(4, sizeof(sf2d_vertex_pos_col));
	if (!vertices) return;
...
void sf2d_draw_fill_circle(int x, int y, int radius, u32 color)
{
	...
	sf2d_vertex_pos_col *vertices = sf2d_pool_calloc(num_segments + 2, sizeof(sf2d_vertex_pos_col));
	if (!vertices) return;
